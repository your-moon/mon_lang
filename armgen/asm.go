...
// Ldrsw32Reg: 32-bit load that SIGN-extends into Xt, [Xn]. mon_lang's тоо is a
// signed Int32, so a 4-byte field holding -1 must become 0xffff...ffff, not
// 0x0000_0000_ffff_ffff — otherwise `x >= 0` guards and signed arithmetic on it
// go wrong.
func (b *Buf) Ldrsw32Reg(rt, rn int) { b.w(0xB9800000 | (uint32(rn) << 5) | uint32(rt)) }

// Str32Reg: 32-bit store, [Xn].
