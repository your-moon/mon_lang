...
func (x X86_64Emitter) EmitMov(src IR, dst IR) {
	x.WriteFile.WriteString(fmt.Sprintf("    movl $%s, -%d(%s)\n", src, Counter, "%rbp"))
}
func (x X86_64Emitter) EmitUnary(op IrUnOp, dst IR) {
}
func (x X86_64Emitter) Emit() {
	...
	for _, ir := range x.Irs {
		...
		default:
			fmt.Println(irtype)
			// case *IRUnary:
			// x.EmitMov(irtype.Src, irtype.Dst)
			// x.WriteFile.WriteString("    # unary\n")
			// x.WriteFile.WriteString(fmt.Sprintf("    movl %s\n", irtype.Dst
